-- Extend record_stripe_checkout_order with discount support (promotion codes, coupons).
-- The old 9-arg function is replaced by an 11-arg version that also receives
-- p_discount_cents and p_coupon_code; callers that omit them default to zero/null.

-- Drop the old 9-arg function before recreating so PostgreSQL resolves the new signature.
drop function if exists public.record_stripe_checkout_order(
  text, text, text, text, text, jsonb, jsonb, integer, text
);
...
revoke all on function public.record_stripe_checkout_order(text, text, text, text, text, jsonb, jsonb, integer, text, integer, text)
  from public, anon, authenticated;
grant execute on function public.record_stripe_checkout_order(text, text, text, text, text, jsonb, jsonb, integer, text, integer, text)
  to service_role;
