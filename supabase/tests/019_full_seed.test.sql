...
-- Every published product is sellable the day the seed runs: from a shelf, or from a funded
-- pre-order allocation (the 2026-09-21 drop).
select results_eq($$select count(*)::integer from public.products where publication_status = 'published'
  ...
  )$$, array[0], 'every published product sells from stock or from a funded pre-order');
