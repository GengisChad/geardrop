...
select results_eq($$select subtotal_cents,discount_cents,shipping_cents,total_cents from public.orders where email='guest@example.com'$$,$$values(2000,100,500,2400)$$,'server recalculates all integer-cent totals');
...
select results_eq($$select product_name_snapshot,unit_price_cents,line_total_cents from public.order_items where order_id=(select id from public.orders where email='guest@example.com')$$,$$values('Original snapshot',1000,2000)$$,'order line stores immutable snapshots');
...
select results_eq($$select product_name_snapshot,unit_price_cents from public.order_items where order_id=(select id from public.orders where email='guest@example.com')$$,$$values('Original snapshot',1000)$$,'product edits never rewrite historical lines');
