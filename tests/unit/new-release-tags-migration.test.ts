...
describe("new release tags", () => {
  it("tags exactly the catalogue's new releases in the database migration", () => {
    ...
    expect(tagged).toEqual(["glory-valkerion-lf", "hurricane-enlil-is-7-55t", "shatter-horus-9-65gb"]);
    const slugList = migration.match(/where product\.slug in \(([^)]+)\)/)?.[1] ?? "";
    expect([...slugList.matchAll(/'([^']+)'/g)].map((match) => match[1]).sort()).toEqual([...tagged].sort());
