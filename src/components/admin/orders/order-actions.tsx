...
export function OrderActions({ orderId, status, paymentStatus, role, tracking, shippingNotifiedAt, stripePaymentIntentId, totalCents }: {
  ...
  const stripeRefundable = refundable && Boolean(stripePaymentIntentId);
  ...
  return <div className={styles.actionGrid}>
    ...
    {stripeRefundable ? <form action={stripeRefundAction} className={`${styles.actionCard} ${styles.dangerCard}`}>
      ...
      <input name="orderId" type="hidden" value={orderId}/>
      <label>Importo EUR (max {(totalCents / 100).toFixed(2)})<input defaultValue={(totalCents / 100).toFixed(2)} max={(totalCents / 100).toFixed(2)} min="0.01" name="amount" required step="0.01" type="number"/></label>
      <label>Motivazione<textarea maxLength={1000} minLength={3} name="reason" required rows={3}/></label>
