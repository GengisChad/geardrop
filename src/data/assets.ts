...
/**
 * Hero energy-impact VFX. Supplied on the same painted checkerboard as the logos;
 * build-assets.py (`cut_glow`) keys it to real transparency and crops to the energy's
 * alpha mass. Size must track the script's output.
 */
export const heroImpact = { src: "/hero/impact.png", width: 1353, height: 830 } as const;
...
/**
 * Product art. Real Beyblade X packshots supplied by the owner in /prodotti, normalised by
 * scripts/normalize_images.py into uniform 1000×1000 WebP tiles: trimmed to the product and
 * centred on a white square with consistent padding, so every card and gallery frame reads
 * the same. Sizes below match the script's output, so next/image never guesses.
 */
