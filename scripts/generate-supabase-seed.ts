...
export function generateSupabaseSeed(): string {
  ...
  const knownProductSlugs = new Set<string>(catalogueSlugs);
  const homepageProductRows = HOMEPAGE_SECTION_SEEDS.flatMap((section) => {
    const requested = section.productSlugs ?? [];
    const productSlugs = requested.every((slug) => knownProductSlugs.has(slug))
      ? requested
      : catalogueSlugs;
    return productSlugs.map((slug, index) => [text(section.key), text(slug), index].join(", "));
  });
